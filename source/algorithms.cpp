...
size_t pbd::algorithms::sort_calculate_needed_helper_list_length(size_t aMaxValueCount)
{
	auto subkeyLength = 4u;
	auto bucketCount = static_cast<uint32_t>(pow(2u, subkeyLength));
	auto elementCount = static_cast<uint32_t>(aMaxValueCount);
	auto groupsize = 512u;
...
size_t pbd::algorithms::prefix_sum_calculate_needed_helper_list_length(size_t aMaxValueCount)
{
	auto elementCount = static_cast<uint32_t>(aMaxValueCount);
	auto groupsize = 512u;
	auto result = 0u;
...
void pbd::algorithms::sort(const avk::buffer& aValues, const avk::buffer& aSecondValues, const avk::buffer& aHelperList, const avk::buffer& aValueCount, size_t aMaxValueCount, const avk::buffer& aResult, const avk::buffer& aSecondResult, uint32_t aValueUpperBound)
{
	auto subkeyLength = 4u;
	auto blocksize = 512u;
	auto bucketCount = static_cast<uint32_t>(pow(2u, subkeyLength));
	auto maxValueCount = static_cast<uint32_t>(aMaxValueCount);
	...
	for (auto subkeyOffset = 0u; (subkeyOffset < 32u) && (aValueUpperBound >> subkeyOffset != 0); subkeyOffset += subkeyLength)
	{
		...
		if (doPrefixSum)
		{
			// histogram table length was written into aHelperList[lengthsAndOffsetsOffset + 0] and aHelperList[lengthsAndOffsetsOffset + 5] by radix_sort_apply_on_block_level(), as well as 0 into aHelperList[lengthsAndOffsetsOffset + 4]
			prefix_sum_recursive(histogramTable, aHelperList, histogramTable, 0u, maxHistogramTableCount);
			if (lastIteration) {
...
{
	auto lengthsAndOffsetsOffset = aHelperList->meta_at_index<avk::buffer_meta>().total_size() / 4 - 10;
	auto maxValueCount = static_cast<uint32_t>(aMaxValueCount);
	...
	prefix_sum_recursive(aValues, aHelperList, aResult == nullptr ? aValues : *aResult, 0u, maxValueCount);
}

void pbd::algorithms::prefix_sum_recursive(const avk::buffer& aValues, const avk::buffer& aHelperList, const avk::buffer& aResult, uint32_t aRecursionDepth, uint32_t aMaxValueCount)
{
	...
	prefix_sum_recursive(aHelperList, aHelperList, aHelperList, aRecursionDepth + 1u, aMaxValueCount);
